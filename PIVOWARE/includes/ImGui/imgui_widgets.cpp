...
bool ImGui::ScrollbarEx(const ImRect& bb_frame, ImGuiID id, ImGuiAxis axis, ImS64* p_scroll_v, ImS64 size_avail_v, ImS64 size_contents_v, ImDrawFlags flags)
{
    ...
    window->DrawList->AddRectFilled(grab_rect.Min, grab_rect.Max, GetColorU32(properties::elements::background_widget_color), 30.f);
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    if (!is_multiline)
    {
        GetWindowDrawList()->AddRectFilled(frame_bb.Min, frame_bb.Max, GetColorU32(properties::elements::background_color), properties::elements::rounding);
        GetWindowDrawList()->AddRect(frame_bb.Min, frame_bb.Max, GetColorU32(properties::elements::background_widget_color), properties::elements::rounding);
    }
    ...
    PushFont(g_pFontManager->GetFont(xorstr_("Lexend-Bold"), 17));
    {
        ...
        if (render_cursor || render_selection)
        {
            ...
            if (render_selection)
            {
                ...
                ImU32 bg_color = GetColorU32(properties::accent_color, 0.1f); // FIXME: current code flow mandate that render_cursor is always true here, we are leaving the transparent one for tests.
            ...
            {
                ImU32 col = GetColorU32(is_displaying_hint ? properties::elements::text_color : properties::elements::text_active_color);
            ...
            if (render_cursor)
            {
                ...
                if (cursor_is_visible && cursor_screen_rect.Overlaps(clip_rect)) draw_window->DrawList->AddLine(ImVec2(total_bb.Min.x + state_anim.slow, cursor_screen_rect.Min.y), ImVec2(total_bb.Min.x + state_anim.slow, cursor_screen_rect.GetBL().y), GetColorU32(properties::elements::text_active_color));
    ...
    GetWindowDrawList()->AddRectFilledMultiColor(frame_bb.Min + ImVec2(1, 1), frame_bb.Max - ImVec2(size_arg.y - CalcTextSize(hint).y - 1, 1), GetColorU32(properties::elements::background_color, 0.f), GetColorU32(properties::elements::background_color, 1.f), GetColorU32(properties::elements::background_color, 1.f), GetColorU32(properties::elements::background_color, 0.f), properties::elements::rounding);

    PushFont(g_pFontManager->GetFont(xorstr_("Icomoon-Widget"), 16));
    GetWindowDrawList()->AddText(frame_bb.Max - ImVec2((size_arg.y + CalcTextSize(label).x) / 2, (size_arg.y + CalcTextSize(label).x) / 2), GetColorU32(properties::accent_color), label);
    PopFont();
